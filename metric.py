...
def inference(loader, model, device, view, data_size,epoch,tem,warmup_epochs):
    model.eval()
    commonZ = []
    labels_vector = []
    for step, (xs, y, _) in enumerate(loader):
        for v in range(view):
            xs[v] = xs[v].to(device)
        with torch.no_grad():
            commonz, _, weight= model.SGDVA(xs,epoch,tem,warmup_epochs)
...
def valid(model, device, dataset, view, data_size, class_num,epoch,tem,warmup_epochs):
    test_loader = DataLoader(
            dataset,
            batch_size=256,
            shuffle=False,
        )
    labels_vector, commonZ = inference(test_loader, model, device, view, data_size,epoch,tem,warmup_epochs)
    print('---------train over---------')
